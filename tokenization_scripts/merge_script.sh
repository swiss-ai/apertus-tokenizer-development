...
while read -r bucket_dumps; do
  ...
  sbatch $RES_OPT \
    ...
    --time=4:00:00 \
    $NO_REQUEUE \
    --job-name=merge-$DATASET_NAME-bucket-$bucket_num \
    --output=$PATH_TO_SLURM_LOGGING_DIR/merge-%x-%j.out \
    --error=$PATH_TO_SLURM_LOGGING_DIR/merge-%x-%j.err \
    --wrap="python3 merge_tokenized_datasets.py --input-dirs ${DUMP_DIRS[@]} --output-prefix $OUTPUT_PREFIX"

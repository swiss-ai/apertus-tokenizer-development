...
- **Val BPB / FLORES BPB**: downstream LM bits per byte; lower = better. The headline FLORES BPB is the macro-mean over the 31 training languages; the 214-language FLORES BPB is reported in REPORT_focus_candidates.md, not here. The FLORES BPB cell shows `mean [lo, hi]` with an across-language 95% CI (mean ± 1.96·stdev/√n); the FLORES BPB σ column reports the across-language stdev (how much BPB varies across languages). Val BPB has no per-language decomposition in the candidates' meta files, so no stdev is shown for it.
...
The recommended tokenizers and the current Apertus production baseline, on the decision metrics. The intrinsic columns are computed on the broad FLORES set. Val BPB, FLORES BPB, MC-math, and MBPP come from the downstream language models. FLORES BPB here is the macro-mean over the 31 FLORES languages in the LM training set; the 214-language FLORES BPB is reported in REPORT_focus_candidates.md, not here. `pending` means the run is mapped but not yet measured, and `-` means not run.
...
- **Parity-aware vs plain BPE.** Parity-aware BPE has a lower Gini than plain BPE (0.081 against 0.114 at matched settings), at a small Eng B/tok cost (4.24 against 4.43). Parity-aware BPE is the better choice here.
...
- **clean-multi vs apertus.** clean-multi has a higher operator-isolation (0.99 against 0.50) and a higher MBPP (0.190 against 0.058). clean-multi is preferable unless multilingual FLORES BPB matters more than code generation.
- **Capped vs uncapped.** The capped regex produces fewer junk tokens (28 against 64) and has no dead-vocab warning, with no change to Eng B/tok or Val BPB. Capping is the better default.
- **Hybrid-window vs base parity.** Hybrid-window has a higher Eng B/tok (4.24 against 3.13) with no fairness gain for base parity. Hybrid-window is the better choice.
...
Two other FLORES corpora appear only in later sections and are distinct from the intrinsic core/broad/full sets above. The downstream LM FLORES BPB set has 214 languages and is used only for the language models' FLORES bits-per-byte; the headline FLORES BPB in this report instead uses the 31 training languages, and the 214-language macro is reported in REPORT_focus_candidates.md. The production-safety vocabulary-usage table is computed on FLORES dev (211 languages) with FineMath-4+ and StarCoder.
...
**Full per-tokenizer results** (point estimates; `[matched]`/`[proxy]`/`pending`/`-` as in the ablations; MBPP/GSM8K/HumanEval are point estimates; the `preliminary_*` candidates have no paired-bootstrap CI, so no MBPP CI is shown for them). **BLiMP is Option-B (BOS / empty-context) scoring for all rows.** The main eval files mix Option-A and Option-B, which are not comparable, so only Option-B is reported; `optA-only` flags a run that has no Option-B eval (its Option-A value is omitted, not substituted):
| Tokenizer | Val BPB ↓ | FLORES tr [95% CI] ↓ | FLORES tr σ ↓ | Code BPB ↓ | BLiMP ↑ | MultiBLiMP ↑ | MGSM ↑ | MC-math ↑ | GSM8K (flex-extract) ↑ | HumanEval ↑ | MBPP ↑ |
|---|---|---|---|---|---|---|---|---|---|---|---|
